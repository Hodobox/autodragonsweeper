...
print(f'{n} games: {len(won)} won, {len(cleared)} cleared ({len(won_failed)} won but failed to clear)')
...
print(f'Had to risk it in {took_risk_won*100:.1f}% won games; {took_risk_won_failed*100:.1f}% when no clear, {took_risk_cleared*100:.1f}% when clear')

def ave_wall_hit(games: list[dict]) -> float:
    hits: int = sum([ g['earlyWallHits'] for g in games ])
    return hits / len(games)

print(f'Average early wall hits: {ave_wall_hit(cleared):.2f} when cleared, {ave_wall_hit(won_failed):.2f} when won without clear, {ave_wall_hit(lost):.2f} when lost')
...
print(f'Average mine king delay: {ave_mk_misses(cleared):.2f} when cleared, {ave_mk_misses(won_failed):.2f} when won without clear')
...
print(f'When won without clear, on average had {ave_dmg_to_go(won_failed):.2f} damage to go')
...
most_hp_occured: int = len([g for g in cleared if g['endStats']['hpLeftOver'] == most_hp_left])
